...
#endif
    Box(TEXT("RailLeft"), FVector(-850.f,0.f,30.f), FVector(35.f,10000.f,20.f));
...
void AQuayCrane::ResetTerminal()
{
    ...
    TrolleyPosition=-2200.f; GantryPosition=-2400.f; RopeLength=1100.f;
    GantryRoot->SetRelativeLocation(FVector(0.f,GantryPosition,0.f),false,nullptr,ETeleportType::TeleportPhysics);
    TrolleyMesh->SetRelativeLocation(FVector(TrolleyPosition,0.f,3000.f),false,nullptr,ETeleportType::TeleportPhysics);
    for (int32 I=0; I<CargoBodies.Num(); ++I)
    {
        CargoOnShip[I]=true;
        ContainerActors[I]->ResetCargo(TerminalSlot(I,true));
    }
    Spreader->SetWorldLocationAndRotation(FVector(TrolleyPosition,GantryPosition,1900.f),FRotator::ZeroRotator,false,nullptr,ETeleportType::TeleportPhysics);
...
void AQuayCrane::StartAutomatic(bool bLoad)
{
    ...
    ResultsCsv=TEXT("ContainerID,Direction,SimulationSeconds,DestinationX,DestinationY,DestinationZ,AGVID\n");
...
bool AQuayCrane::DriveSpreaderTo(FVector Target)
{
    ...
    SetDriveInput((0.8f*(Target.X-TrolleyPosition)-0.8f*DriveVelocity.X)/TravelSpeed,
        ...
        (0.8f*(RopeLength-TargetRope)-0.8f*DriveVelocity.Z)/HoistSpeed);
    return FMath::Abs(Target.X-TrolleyPosition)<5.f && FMath::Abs(Target.Y-GantryPosition)<5.f &&
        FMath::Abs(RopeLength-TargetRope)<4.f && DriveVelocity.Size()<7.f;
}
...
void AQuayCrane::TickAutomatic(float Dt)
{
    ...
    case ETerminalStage::RaiseEmpty:
        if (DriveSpreaderTo(Target) && Spreader->GetComponentLocation().Z>Terminal::SafeHeight-25.f) SetAutoStage(ETerminalStage::Approach);
        break;
    case ETerminalStage::Approach:
        Target=FVector(AutoSource.X,AutoSource.Y,Terminal::SafeHeight);
        bReady=DriveSpreaderTo(Target) && FVector::Dist2D(Spreader->GetComponentLocation(),Target)<25.f && Spreader->GetPhysicsLinearVelocity().Size()<20.f;
    ...
    case ETerminalStage::LowerPickup:
        Target=Cargo->GetComponentLocation()+FVector(0.f,0.f,Terminal::HalfHeight+25.f);
        bReady=DriveSpreaderTo(Target) && FVector::Dist(Spreader->GetComponentLocation(),Target)<30.f && Spreader->GetPhysicsLinearVelocity().Size()<15.f;
        AutoStableTime=bReady?AutoStableTime+Dt:0.f;
        if (AutoStableTime>0.5f)
        {
            ToggleLock();
    ...
    case ETerminalStage::Lift:
        Target=FVector(AutoSource.X,AutoSource.Y,Terminal::SafeHeight);
        if (DriveSpreaderTo(Target) && Cargo->GetComponentLocation().Z>1600.f) SetAutoStage(ETerminalStage::Transfer);
        break;
    case ETerminalStage::Transfer:
        Target=FVector(AutoDestination.X,AutoDestination.Y,Terminal::SafeHeight);
        bReady=DriveSpreaderTo(Target) && FVector::Dist2D(Cargo->GetComponentLocation(),Target)<20.f && Cargo->GetPhysicsLinearVelocity().Size()<15.f;
        AutoStableTime=bReady?AutoStableTime+Dt:0.f;
        if (AutoStableTime>0.6f)
        {
            ...
            if (!bAutoLoading && (AGVActors[ActiveAGV]->Speed>0 || bAGVHasCargo || FVector::Dist(AGVCargoPosition(),AutoDestination)>1.f))
    ...
    case ETerminalStage::LowerPlace:
        ...
        bReady=DriveSpreaderTo(Target) && FVector::Dist(Cargo->GetComponentLocation(),AutoDestination)<18.f && Cargo->GetPhysicsLinearVelocity().Size()<12.f;
        AutoStableTime=bReady?AutoStableTime+Dt:0.f;
        if (AutoStableTime>0.7f) { ToggleLock(); SetAutoStage(ETerminalStage::Retract); }
        break;
    ...
    case ETerminalStage::Verify:
        bReady=!bLocked && FVector::Dist(Cargo->GetComponentLocation(),AutoDestination)<20.f &&
            Cargo->GetPhysicsLinearVelocity().Size()<8.f && Cargo->GetUpVector().Z>0.99f;
        AutoStableTime=bReady?AutoStableTime+Dt:0.f;
...
void AQuayCrane::CompleteAutomaticJob()
{
    ...
    ResultsCsv+=FString::Printf(TEXT("C%02d,%s,%.3f,%.1f,%.1f,%.1f,AGV%d\n"),ActiveCargoIndex+1,
        bAutoLoading?TEXT("Load"):TEXT("Unload"),JobElapsed,Destination.X,Destination.Y,Destination.Z,ActiveAGV+1);
    IFileManager::Get().MakeDirectory(*FPaths::GetPath(ResultsPath),true);
    if (!FFileHelper::SaveStringToFile(ResultsCsv,*ResultsPath,FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
